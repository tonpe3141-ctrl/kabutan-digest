...
# 想定オープンに使う日経平均先物。CNBC の日足 API（cnbc.fetch_bars）で取る。
# CME のドル建て・期近。日足の日付は米国の取引日（ET）で、終値は清算値（16:00 ET ごろ
# ＝翌朝 05:00〜06:00 JST）。2026-08〜09 の 34 営業日で「清算値 − 前日終値」を実際の寄りと
# 比べると平均絶対誤差は約300円（固定係数モデルは約390円）、平均のずれは +25円でベーシスは小さい。
NIKKEI_FUTURES = {"symbol": "@NK.1", "label": "CME日経平均先物（期近）"}
FUTURES_MAX_AGE_DAYS = 2     # 07:10 JST 時点の米国の日付から数えて、これより古い足は使わない
